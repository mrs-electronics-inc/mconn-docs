...
By default, the Ethernet interface is configured as a **DHCP Server**. You can switch between **DHCP Server** and **DHCP Client** modes using the `eth-conf.sh` utility.

To configure `eth0` as a **DHCP Client** (to connect the display to an existing network):
```shell
eth-conf.sh -c
```

To configure `eth0` as a **DHCP Server** (default, for direct connection to the display):
```shell
eth-conf.sh -s
```
:::tip
A reboot is required after switching modes for the changes to take effect:
:::

:::caution
Switching modes will change the IP address of the display. If you are connected via SSH, your session will be dropped. 
When in Server mode, connect directly to the display at `192.168.57.1`. 
When in Client mode, find the assigned IP using a Network Analyzer app and look for the hostname prefix `imx6qmrsdisp`.
:::
