...
To connect via Ethernet, simply plug the **RJ45 Ethernet cable** into the port at the back of the device. By default, the display Ethernet interface is configured as a **DHCP client**, meaning it will automatically obtain an IP address from your network's DHCP server (typically your router or network switch).

Since the IP address is dynamically assigned, you will need to find it before connecting. The easiest ways to do this are:

- Use a Network Analyzer app (e.g. Fing, Advanced IP Scanner, or similar) to scan your network. Look for a device with the hostname prefix `imx6qmrsdisp`.
- Alternatively, scan the available IPs on your network and look for the device by its MAC address or hostname.
- Check your router's **DHCP lease table** for the assigned IP.

Once you have the IP address, you can connect via SSH:
```shell
ssh root@<assigned-ip>
```
For example:
```shell
ssh root@10.20.10.38
```

### SSH/SFTP Connection

The connection to Display via **SSH/SFTP** is required to access the terminal or perform actions like sharing files. The connection details are as follows:

| **Field**                 | **Value**                                                                 |
| ------------------------- | ------------------------------------------------------------------------- |
| **IP Address (Ethernet)** | Dynamic — find using Network Analyzer (see above) <br/> Example: `192.168.1.38` |
| **Port**                  | 22                                                                        |
| **Username**              | root                                                                      |
| **Password**              | mrsroot                                                                   |
...
To connect via USB to Ethernet, you need an RJ45 to USB-A adapter. Plug the USB-A end into one of the USB ports at the back of the device and connect the RJ45 end via an Ethernet cable to your network. The display in this configuration also acts as a DHCP client — ensure you have a DHCP server in your network. Find the IP address assigned to the display using a Network Analyzer tool as described above or your router's DHCP lease table, then connect via SSH.
## File System
