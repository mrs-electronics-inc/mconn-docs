...
:::caution
This change is not persistent across reboots. Since the root filesystem is read-only, the name resets after a reboot. To make it persistent, remount the filesystem as writable and set the name in a startup script:
...
**Step 1 — Launch bluetoothctl and prepare the display:**
...
**Step 2 — Initiate pairing from your phone or PC**
...
**Step 4 — Authorize services if prompted**
...
:::tip
To pair from the command line without entering the interactive shell, you can run individual commands directly:
...
- **Custom GATT server** — expose custom BLE services and characteristics to connected devices using `btgatt-server`, useful for building BLE-based data interfaces between the display and other embedded systems
