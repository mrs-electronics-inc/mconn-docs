...
:::tip
These steps need to be run each time the device boots. To automate this, add the commands to `/usr/bin/startup-app.sh`. Since the root filesystem is read-only, remount it as writable first:
...
:::tip
This change is not persistent across reboots — the name, discoverability, and pairing settings all reset after a reboot. To make them persistent, add the following lines to `/usr/bin/startup-app.sh`. Since the root filesystem is read-only, remount it as writable first:
...
:::tip
These settings reset on every reboot. To make them persistent, add them to `/usr/bin/startup-app.sh`:
```bash
mount -o remount,rw /
nano /usr/bin/startup-app.sh
```
